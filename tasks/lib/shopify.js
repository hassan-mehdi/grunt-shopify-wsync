...
module.exports = function(grunt) {
    ...
    shopify._getSyncThemeId = function() {
        var config = grunt.config('shopify');
    ...
    shopify.sync = function(filepath, done) {
        var api = shopify._getApi(),
            themeId = shopify._getSyncThemeId(),
            key = shopify._makeAssetKey(filepath);
     ...
     * @param {Function} done
     */
    shopify.syncTheme = function(done) {
        var api = shopify._getApi();
        var themeId = shopify._getSyncThemeId(),
            basePath = shopify._getBasePath();
        ...
        function onRetrieve(err, obj) {
            ...
            async.eachSeries(obj.assets, function(asset, next) {
                shopify.sync(path.join(basePath, asset.key), next);
            }, function(err) {
                if (!err) {
                    shopify.notify('Theme download complete.');
                }
